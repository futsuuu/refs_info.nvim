...
local utils = require 'refs_info.utils'
local params = require 'refs_info.params'
local config = require 'refs_info.config'
local VirtLine = require 'refs_info.virt_line'

local ns = api.nvim_create_namespace 'refs_info'

---@class refs_info.Main
---@field buf buffer
---@field running boolean
---@field cancel_funcs function[]
---@field virt_line refs_info.VirtLine
local M = {}

---@param buf buffer
---@return refs_info.Main?
function M.new(buf)
...
function M:get_document_symbols(callback)
  ...
  if not utils.support_method(self.buf, method) then
    return
  end
  ...
  local cancel = lsp.buf_request_all(
    ...
    utils.get_all_results(
      ---@param results lsp.DocumentSymbol[][]
      function(results)
        ---@type lsp.DocumentSymbol[]
        local symbols = utils.flatten(results, 1)
...
function M:get_references(position, callback)
  ...
  local cancel = lsp.buf_request_all(
    ...
    utils.get_all_results(function(results)
      callback(utils.flatten(results, 1))
    end)
  )
